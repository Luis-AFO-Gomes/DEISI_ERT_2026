...
├── deprecated/                 # Deprecated code and artefacts 
...
- **deprecated/** — Contains code and artefacts that are no longer in use but are kept for reference or learning purposes. This may include old versions of requirements, test cases, or code that has been refactored. It might also contain artefacts not in use in present version but kept for future reuse
...
The recommended virtual environment folder name is `.venv`, but any other can be used.<br> 
If using a different name, update the virtual environment commands below accordingly. <br>
<br>
Virtual environment usage commands will vary based on the operating system and shell used.
...
.venv\Scripts\activate
```
Windows might restrict script running in powershell for security reasons. In this case, restrict policy should be changed to allow virtual environment start.<br>
