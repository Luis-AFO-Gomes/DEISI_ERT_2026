...
### Scenario: Apresentar catálogo com indicação de disponibilidade

**GIVEN** que a lista de títulos está carregada no ecrã  
**THEN** cada título apresenta uma indicação clara de disponibilidade para empréstimo

### Scenario: Manter indicação de disponibilidade após ordenação

**GIVEN** que a lista foi ordenada  
**WHEN** o utilizador visualiza a lista ordenada  
**THEN** a lista mantém os mesmos campos da lista padrão, incluíndo a indicação de disponibilidade  
**AND** mantém as mesmas opções disponíveis  
**AND** mantém a mesma navegabilidade  
